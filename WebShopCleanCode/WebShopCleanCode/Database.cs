...
namespace WebShopCleanCode
{
    public class Database
    {
        ...
        {
            List<ProductProxy> proxies= new List<ProductProxy>();
            foreach(Product product in productsInDatabase)
            {
                proxies.Add(new ProductProxy(product.Name,product.Price, this));
            }
            return proxies;
        }
        public Product GetProductByName(string name)
        {
            foreach(Product product in productsInDatabase)
            {
                if(product.Name.Contains(name)) return product;
            }
            return null;
        }
    }
